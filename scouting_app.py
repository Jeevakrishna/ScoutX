...
_WORDMARK_NAME = "Player HUNTER!!" if _lang() == "en" else "S"
_WORDMARK_TAG  = "Euro Top-5 Scouting Database" if _lang() == "en" else ""
st.sidebar.markdown(
    f'''
    <div class="ct-wordmark"> <svg width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="/Scout/favicon.svg"> <!-- Pitch outline --> <rect x="2" y="6" width="30" height="22" rx="2" stroke="#5DA5E8" stroke-width="1.4" fill="rgba(93,165,232,0.06)"/> <!-- Centre line --> <line x1="17" y1="6" x2="17" y2="28" stroke="#5DA5E8" stroke-width="1.2" opacity="0.7"/> <!-- Centre circle --> <circle cx="17" cy="17" r="3.5" stroke="#5DA5E8" stroke-width="1.2" fill="none" opacity="0.7"/> <!-- Penalty boxes --> <rect x="2" y="11" width="4.5" height="12" stroke="#5DA5E8" stroke-width="1.2" fill="none" opacity="0.7"/> <rect x="27.5" y="11" width="4.5" height="12" stroke="#5DA5E8" stroke-width="1.2" fill="none" opacity="0.7"/> <!-- Scouting target dot --> <circle cx="17" cy="17" r="1.6" fill="#5DA5E8"/> </svg> <div class="ct-wordmark-text"> <span class="ct-wordmark-name">{_WORDMARK_NAME}</span> <span class="ct-wordmark-tag">{_WORDMARK_TAG}</span> </div> </div>
    '''
,
    unsafe_allow_html=True,
)
...
st.markdown('<div class="top-control-bar desktop-only">', unsafe_allow_html=True)
...
if "side_nav_radio" not in st.session_state:
    st.session_state.side_nav_radio = PAGE_KEYS[0]

def sync_top():
    st.session_state.side_nav_radio = st.session_state.nav_radio
def sync_side():
    st.session_state.nav_radio = st.session_state.side_nav_radio

# Render Top Nav (Desktop)
st.markdown('<div class="nav-container-fixed desktop-only">', unsafe_allow_html=True)
with st.container():
    st.radio(
        "Navigation",
        PAGE_KEYS,
